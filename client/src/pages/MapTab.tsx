...
import { Loader2, Snail as SnailIcon } from "lucide-react";
import { Coordinates, getSnailPosition, isInInterceptRange, haversineDistance, getDefaultArrivalTime, coordinatesToPathJson } from "@shared/ghostMovement";
...
import { createCirclePolygon, parseSupabasePoint, projectPointToCircle } from "@/lib/geo";
import { INTERCEPT_RANGE_METERS, SNAIL_FOCUS_EVENT, SNAIL_FOCUS_STORAGE_KEY, HOME_ZONE_RADIUS_KM, MIN_DEPLOY_DISTANCE_METERS, SNAIL_TRAVEL_DURATION_HOURS } from "@shared/const";
...
export default function MapTab() {
  const { user } = useAuth();
  const { incomingSnails, outgoingSnails, snails, refresh: refreshSnails, interceptSnail, deploySnail } = useSnails();
  ...
  return (
    <div className="relative w-full h-screen">
      ...
      {/* Demo mode controls */}
      <div className="absolute bottom-20 right-6 pointer-events-none flex flex-col items-end gap-2">
        <Button
          onClick={toggleDemoMode}
          variant={isDemoMode ? "default" : "outline"}
          size="sm"
          className={`pointer-events-auto rounded-full px-4 py-1 h-8 ${
            isDemoMode ? "" : "bg-white text-blue-500 hover:bg-gray-50 border-2 border-gray-300"
          }`}
        >
      ...
      {/* Release Snail button */}
      <div className="absolute bottom-20 left-6">
        <Button
          onClick={handleOpenDeploy}
          size="lg"
          className="h-20 w-20 rounded-full bg-white hover:bg-gray-100 shadow-lg border-2 border-gray-300 p-3"
        >
          <img src="/user-snail.webp" alt="Release Snail" className="w-full h-full object-contain" />
        </Button>
      </div>
